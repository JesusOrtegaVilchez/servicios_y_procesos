...
void fill(Queue *list, int n,const char *name){
    ...
    for(int i = 0; i<n;i++){
        ...
        new_product.price = (rand()%100)/100.;
...
void print(Queue q){
    for(int i=q.init; i<q.summit;i++)
        printf("%s:  %s - %.2lf (%.2lf)\n",q.name
                q.data[i].name,q.data.price,q.data[i].off);
}
void *lady(void *data){
    ...
    for(int i=0;i<1000;i++){
        bargain_number = rand()%2;    
        grab = shift(&bargain[bargain_number]);
...
int main(){
    Queue bargain;//gangas o ofertas
    pthread_t thr_id[5];
    const char *name[]={
        "Josefa",
        "Maria",
    ...
    init(&bargain[0]);
    init(&bargain[1]);
    fill(&bargain,100,"The english cut");
    fill(&bargain,100,"Apreciated Halls");
